...
function App() {
  ...
  const [isRegistering, setIsRegistering] = useState(false); // Estado para alternar entre formularios
  ...
  const fetchWeatherByCity = async (cityOrLat, lon = null) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/weather/currentAndForecast?${
          lon ? `lat=${cityOrLat}&lon=${lon}` : `city=${cityOrLat}`
        }`,
        {
          method: 'GET',
          headers: {
        ...
        {loggedIn ? (
          <div className="weather-container">
            ...
            {currentWeather.dailyForecast &&
              currentWeather.dailyForecast.length > 0 && (
                <button
                  ...
                  onClick={exportDailyForecastReport}
                >
                  Exportar reporte
                </button>
              )}
            <div className="current-weather">
              <h2>
                Clima Actual en {currentWeather.city}, {currentWeather.country}
              </h2>
            ...
            {currentWeather.dailyForecast && (
              <div className="days-forecast">
                ...
                <ul className="weather-cards">
                  {Array.isArray(currentWeather.dailyForecast) &&
                  currentWeather.dailyForecast.length > 0 ? (
                    currentWeather.dailyForecast.map((day, index) => (
                      <li key={index} className="card">
                        ...
                        <div className="forecast-details">
                          ...
                          <h6>
                            Nubes:{' '}
                            {day.clouds !== undefined ? day.clouds : 'N/A°C'}
                          </h6>
                          <h6>
                            Vientos:{' '}
          ...
          <div className="auth-container">
            {isRegistering ? ( // Mostrar formulario basado en isRegistering
